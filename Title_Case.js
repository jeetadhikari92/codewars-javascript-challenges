...
function titleCase(title, minorWords) {
  if(title.length > 0 && title.split(' ').length > 1){
    minorWords = minorWords ? minorWords.toLowerCase().split(' ') : '';
    const res = (title.split(' ')
                      .map(a => !minorWords.includes(a.toLowerCase()) ? 
                                 (a[0].toUpperCase() + a.substr(1).toLowerCase()) : a.toLowerCase() 
                       )).join(' ');
    return res[0].toUpperCase() + res.substr(1);   
  } else {
    return title ? title[0].toUpperCase() + title.substr(1).toLowerCase() : title;
  }
